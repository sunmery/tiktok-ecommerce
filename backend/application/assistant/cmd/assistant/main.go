...
func init() {
	...
	flag.StringVar(&configCenter, "configCenter", "localhost:8500", "config center url, eg: -config_center 127.0.0.1:8500")
	flag.StringVar(&configPath, "configPath", "ecommerce/user/config.yaml", "config center path, eg: -config_center ecommerce/user/account/config.yaml")
...
func newApp(logger log.Logger, gs *grpc.Server, hs *http.Server, r registry.Registrar) *kratos.App {
	return kratos.New(
		kratos.ID(fmt.Sprintf("%s-%s", Name, Version)),
		kratos.Name(Name),
