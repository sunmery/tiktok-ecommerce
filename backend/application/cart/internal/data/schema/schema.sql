...
(
    ...
    CONSTRAINT unique_owner_name_cart_name UNIQUE (user_id, cart_name) -- 保证 owner、name 和 cart_name 的组合唯一
);

-- 为 owner、name 和 cart_name 字段创建联合索引
CREATE INDEX idx_owner_name_cart_name ON carts.carts (user_id, cart_name);
...
CREATE INDEX idx_cart_id_product_id ON carts.cart_items (cart_id, product_id);
