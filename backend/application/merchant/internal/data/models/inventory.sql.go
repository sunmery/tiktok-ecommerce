...
//	FROM merchant.stock_adjustments sa
//	  WHERE sa.merchant_id = $1::uuid
func (q *Queries) CountStockAdjustmentHistory(ctx context.Context, merchantID uuid.UUID) (int64, error) {
	row := q.db.QueryRow(ctx, CountStockAdjustmentHistory, merchantID)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
func (q *Queries) GetStockAdjustmentHistory(ctx context.Context, arg GetStockAdjustmentHistoryParams) ([]GetStockAdjustmentHistoryRow, error) {
	rows, err := q.db.Query(ctx, GetStockAdjustmentHistory, arg.ProductID, arg.Page, arg.PageSize)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
